...
def route_to_vendor(method: str, *args, **kwargs):
    ...
    logger.debug(
        "%s - Primary: [%s] | Full fallback order: [%s]", method, primary_str, fallback_str
    )
    ...
    for vendor in fallback_vendors:
        ...
        for impl_func, vendor_name in vendor_methods:
            try:
                ...
                logger.debug(
                    "%s from vendor '%s' completed successfully", impl_func.__name__, vendor_name
                )
                ...
                logger.warning("%s from vendor '%s' failed: %s", impl_func.__name__, vendor_name, e)
                continue

        # Add this vendor's results
        if vendor_results:
            results.extend(vendor_results)
            logger.debug("Vendor '%s' succeeded - Got %d result(s)", vendor, len(vendor_results))
